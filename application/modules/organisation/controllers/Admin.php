...
class Admin extends Admin_Controller {
	function __construct() {
		...
		$this->load->dbforge();
		$this->load->model('organisation_mdl');
	...
	public function usrlist(){
	  	$this->body= array();
	    $this->body['data']=$this->organisation_mdl->get_all_data();
	    //admin check
  	...
  	public function add_organisation(){
	 	$this->data=array();
	 	$this->form_validation->set_rules('name', 'Uaser  Name Or Email', 'trim|required');
		...
		if ($this->form_validation->run() == TRUE){
			...
			$password=$this->input->post('password');
	      	$data=array(
	        	...
	        	'salt'=>$salt,
	        	'type'=>'2', //type two fro organisation project added members this manage in database tables groups
	      	);
	      	$insert=$this->organisation_mdl->add_organisationuser('organisation',$data);
	      	if($insert!=""){
		        $this->session->set_flashdata('msg','User Create successfully !!!!');
		        redirect(FOLDER_ADMIN.'/whodoes/add_organisation');
	        }
	    }else{
	      	//admin check
	    	...
	    	//print_r($id);die;
	    	if($id) {
				$this->data['drrdataeditdata'] = $this->general->get_tbl_data_result('id,username','organisation',array('id'=>$id));
	    	}else{
	    		$this->data['drrdataeditdata'] = array();	
	    	}
	    	$this->data['publicationdata'] = $this->general->get_tbl_data_result('id,username','organisation');	
	      	$admin_type=$this->session->userdata('user_type');
